...
| 0.40 | Setup gains a fifth part, "Upgrading", at the principal's request before 4.0: `forge-pull` as the upgrade of the engine, the projects untouched; `RELEASE-NOTES.md` and its Action required lines first; then `/check project` per project and the migration agreed with Claude one finding at a time, made on the user's word, no migration tool (POS.0940, DEC.0100); a project left as it is stays valid (POS.0820). The reviewers section (8) recast: one subsection per kind of reviewer — critic, challenger, check, and every kind that comes after — each with its roster as a bullet list taken from the agents' descriptions, so that a new lens, persona or check is a new bullet, never a rewritten paragraph; the states of findings as a list per kind. "Saving and syncing" says `/save` runs the light check (POS.1100 since 3.44). | Claude | 2026-09-06 |
| 0.39 | The last two counts of the reviewers dropped from the section instructions ("the two reviewers", "the two are one shape"), and the Template placeholder of the Commands section names `/release` beside `/ingest` and `/save`, as the instruction above it already did (bare `/check-forge` of 2026-09-06 after the migration to skills). | Claude | 2026-09-06 |

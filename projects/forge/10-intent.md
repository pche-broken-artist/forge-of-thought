...
date: 2026-09-05
status: draft
last_change: 3.34 (2026-09-05): check-forge findings of the first /release settled — /release ends by running /save; /release without a slug asks; five positions and two threads say release where they said save (POS.0310, POS.0550, POS.0930, POS.1070, THR.0210, THR.0270); scripts count dropped, forge-status reports the branch.
project: forge
...
- **POS.0730** Release notes stay continuously current. `RELEASE-NOTES.md`
  ...
  Version History. The Unreleased head is regenerated by every `/release` of the
  engine (POS.1100), alongside the README,
...
- **POS.0310** Every versioned document keeps its Version History
  ...
  never by hand, so the two cannot drift. The row in the companion is
  the single primary: the commit messages `/save` and `/release` draft
  and the Unreleased head of the release notes are derivations by
  mechanism,
...
- **POS.1100** Save and release are two commands. `/save` is commit
  ...
  number is the intent's version; the release notes keep their shape
  (an Unreleased head, one section per major). Tags: at every release of

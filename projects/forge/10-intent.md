...
- **POS.0180** External inputs (transcripts, offers, documents,
  ...
  plain slug filenames, binary formats accompanied by a text extract.
  ...
  transcription. `/ingest` runs the script file by file with
  `-Suffix '.extract'` on every binary format that needs an extract,
  in bundles as well as for isolated files; the output lands directly
  as the `.extract.md` sibling.
...
- **POS.0810** Regenerated renders pass under the principal's eyes.
  ...
  delta before the commit without reading a full diff.

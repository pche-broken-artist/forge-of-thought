...
- **POS.0310** Every versioned document keeps its Version History
  ...
  the single primary: the commit message `/save` drafts and the
  Unreleased head of the release notes are derivations by mechanism,
...
- **POS.0550** The engine is persisted in git with a remote of its own,
  ...
  needs, never a rule (the count stated here went stale twice). Three
  of them serve the engine and every project that is a repository (`projects/<slug>/.git`): `forge-save.ps1`
  ...
  commit, origin or "not under git"; changes nothing).
...
- **THR.0270** The shared reviewer skeleton as a preloaded skill instead
  ...
  `/check-forge` compares the copies at every save. Proposed solution:
